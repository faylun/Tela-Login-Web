...
app.post('/register', (req, res) => {
    ...
    if (user === 'admin' && pass === 'admin') {
        ...
        if (rememberMe) {
            req.session.cookie.maxAge = 3600 * 24 * 3 * 1000; // Define o tempo de expiração do cookie se "lembrar-me" estiver ativado
        } else {
...
app.get('/errorLogin', (req, res) => {
    console.log(req.session.user); // Log para depuração

    if (!req.session.user) {
        console.log('Usuário não encontrado'); // Mensagem de depuração
        res.status(200).sendFile(path.join(__dirname, 'views', 'semPermissao.html')); // Mostra a página de acesso negado
    } else {
        console.log('Usuário encontrado'); // Mensagem de depuração
        req.session.destroy(err => {
            if (err) {
                return res.status(500).send('Erro ao encerrar a sessão.'); // Trata erro ao destruir a sessão
            }
